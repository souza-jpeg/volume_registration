...
from irtk.upscalling.esrgan.generate_png_data import get_volumes_folders, get_dataset_random_split, \
    get_files_in_subfolder, create_dataset_folders, central_crop
...
#             slice_croped = central_crop(
#                 img_array=slice_i, 
#                 crop_shape=crop_shape
#             )
#             img = Image.fromarray(slice_croped)
...
for val_subfolder in val_subfolders:
    ...
    for file_path in path_to_files:
        ...
        for i in range(volume.shape[0]):
            ...
            img = Image.fromarray(slice_croped)
            img.save(os.path.join(path_to_save_image, f"{val_subfolder}_{i}.png"))
...
for test_subfolder in test_subfolders:
    ...
    for file_path in path_to_files:
        ...
        for i in range(volume.shape[0]):
            ...
            img = Image.fromarray(slice_croped)
            img.save(os.path.join(path_to_save_image, f"{test_subfolder}_{i}.png"))
